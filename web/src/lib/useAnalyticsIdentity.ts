...
// cuando hay usuario, lo identifica (id estable = uuid de Auth) y emite `login` /
// `signup_completed`; al cerrar sesión, resetea. Se monta UNA vez en App.tsx,
// dentro del árbol con <AuthProvider> (usa useSession()).
...
export function useAnalyticsIdentity(): void {
  ...
  useEffect(() => {
    if (!user) {
      // Logout (había alguien identificado y ahora no): desvincular.
    ...
    // `profile` puede llegar después del user; no queremos re-disparar por eso.
    // El identify ya se hizo con lo disponible; un cambio de display_name no debe
    // contar como nuevo login.
